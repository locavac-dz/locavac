...
describe('DELETE /api/admin/users/:id — anonymisation RGPD', () => {
  ...
  test('garde-fou du parseur de schéma : colonnes connues présentes', () => {
    for (const col of ['id', 'email', 'banned', 'rib', 'ccp', 'id_document', 'id_verified', 'verification_token', 'google_id'])
      expect(USERS_COLUMNS).toContain(col);
  });

  test('toute colonne écrite lors de l\'anonymisation existe réellement dans la table users', async () => {
    await request(app).delete('/api/admin/users/2').set(ADMIN_AUTH);
    const [, changes] = db.users.updateById.mock.calls[0];
    expect(changes).not.toHaveProperty('reset_token');
    const unknown = Object.keys(changes).filter(c => !USERS_COLUMNS.has(c));
